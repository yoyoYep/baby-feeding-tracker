...
1. 打开微信开发者工具
2. 选择「导入项目」
3. 项目目录选择本仓库根目录。如果是从 Git 新拉的代码，先复制 `project.config.example.json` 为 `project.config.json`
4. AppID 填入你自己的小程序 AppID（`project.config.json` 是本地真实配置，不提交 Git）
5. 点击「导入」
...
3. 依次创建以下集合（点「+」号）：
   ...
   - `family_members` — 家庭成员
   - `config` — 共享配置（每顿奶量、喂奶间隔等）
   - `todos` — 每日待办规则
4. 每个集合的权限设为 **「所有用户可读写」**：集合 → 权限设置 → 选择「自定义安全规则」，将内容改为 `{"read": true, "write": true}`

> 如果曾在未创建 `todos` 集合时添加过待办，它们会先保存在本机本地缓存。创建 `todos` 集合并重新打开待办页后，小程序会自动迁移这些本地待办到云端。
...
4. 数据库集合 `records`/`baby`/`med_history`/`family_members`/`config`/`todos` 已创建且权限正确
...
在「我的」→「喂养配置」中设置的每顿奶量和喂奶间隔阈值是**全家共享**的，任何成员修改后所有人生效。
...
真实配置不要提交到公开仓库，当前 `.gitignore` 已屏蔽：
...
确认上面 5 个真实配置文件没有出现在待提交列表后，再执行 `git commit` 和 `git push`。如果误加入暂存区，用 `git rm --cached <文件路径>` 移出暂存，不要删除本地真实配置。
...
│   ├── utils/growth-standard.js ← 生长页/首页时间线 WHO LMS 百分位计算
│   ├── components/voice-input/ ← 语音输入组件
│   ├── components/top-nav/    ← 自定义顶部标题栏
│   └── pages/                 ← 各页面（含 todo / todo-edit 待办页面）
├── cloudfunctions/
│   ├── speechToText/
...
│   └── familyManage/          ← 家庭成员管理
├── tests/                     ← 本地测试脚本
├── .gitignore                 ← 已排除密钥文件和本地项目配置
├── project.config.example.json ← Git 提交用模板（appid=touristappid）
└── project.config.json        ← 本地真实配置（不提交 Git）
```
