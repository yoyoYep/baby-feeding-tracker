...
Page({
  ...
  onLoad(options) {
    const { type = 'feeding', mode = 'timer', id } = options
    ...
    wx.setNavigationBarTitle({ title: navTitle })
    this.setData({ type, mode, navTitle, recordDate: todayStr })
  ...
  async saveSleepRecord() {
    const { ongoingSleepId, sleepEndTime, sleepType, sleepMethod, sleepQuality, wakeCount, note, editMode, editRecordId, manualTime, manualEndTime, recordDate } = this.data
  ...
  async saveTempRecord() {
    const { tempValue, tempMethod, note, editMode, editRecordId } = this.data
    if (!tempValue) {
    ...
    try {
      if (editMode) {
        ...
        await db.addRecord({ type: 'health_temp', startTime: new Date(), data: recordData, status: 'completed', source: 'manual', note })
      }
      wx.showToast({ title: '保存成功', icon: 'success' })
      setTimeout(() => wx.navigateBack(), 1500)
    } catch (e) {
  ...
  async saveMedRecord() {
    const { medName, medDosage, medUnit, medMethod, note, editMode, editRecordId } = this.data
    if (!medName) {
    ...
    try {
      if (editMode) {
        ...
        await db.addRecord({ type: 'health_med', startTime: new Date(), data: recordData, status: 'completed', source: 'manual', note })
      }
      wx.showToast({ title: '保存成功', icon: 'success' })
      setTimeout(() => wx.navigateBack(), 1500)
    } catch (e) {
  ...
  async saveGrowthRecord() {
    const { growthWeight, growthHeight, growthHc, note, editMode, editRecordId, recordDate } = this.data
    ...
    const baseDate = recordDate ? new Date(recordDate) : new Date()
    const startTime = new Date(baseDate.getFullYear(), baseDate.getMonth(), baseDate.getDate(), new Date().getHours(), new Date().getMinutes())

    try {
      if (editMode) {
        await db.updateRecord(editRecordId, { startTime, data, note })
      } else {
        await db.addRecord({ type: 'growth', startTime, data, status: 'completed', source: 'manual', note })
      }
      wx.showToast({ title: '保存成功', icon: 'success' })
      setTimeout(() => wx.navigateBack(), 1500)
    } catch (e) {
