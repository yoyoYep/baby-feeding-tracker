...
const { getPercentile } = require('../../utils/who-data')
...
Page({
  ...
  async loadDayData() {
    try {
      ...
      const timeline = this._formatTimeline(records)
  ...
  async updateLastFeedingTimer() {
    try {
      ...
      if (res.data && res.data.length > 0) {
        ...
        this.setData({ lastFeedingAgo: '', feedingOverdue: false })
      }
    } catch (e) {
      console.error(e)
    }
      ...
      switch (r.type) {
        ...
        case 'growth':
          ...
          if (baby && baby.birthday) {
            monthAge = (new Date(r.startTime) - new Date(baby.birthday)) / (30.44 * 24 * 60 * 60 * 1000)
          }
          if (r.data && r.data.weight) {
            const p = getPercentile(r.data.weight, monthAge, gender, 'weight')
            gParts.push(`${r.data.weight}kg(${p.label})`)
          }
          if (r.data && r.data.height) {
            const p = getPercentile(r.data.height, monthAge, gender, 'length')
            gParts.push(`${r.data.height}cm(${p.label})`)
          }
          if (r.data && r.data.headCirc) {
            const p = getPercentile(r.data.headCirc, monthAge, gender, 'hc')
            gParts.push(`头围${r.data.headCirc}cm(${p.label})`)
          }
          desc = gParts.join(' ')
          break
      }
  ...
  async _handleFeedingStart(result) {
    ...
    try {
      ...
      wx.showToast({ title: '开始喂奶', icon: 'success' })
      await this._refreshHomeData()
    } catch (e) {
      wx.showToast({ title: '记录失败', icon: 'none' })
    }
  ...
  async _handleFeedingEnd(result) {
    ...
    if (this.data.ongoingFeeding) {
      try {
        ...
        wx.showToast({ title: '喂奶结束', icon: 'success' })
        await this._refreshHomeData()
      } catch (e) {
  ...
  async _handleSleepStart(result) {
    try {
      ...
      wx.showToast({ title: '宝宝睡了', icon: 'success' })
      await this._refreshHomeData()
    } catch (e) {
      wx.showToast({ title: '记录失败', icon: 'none' })
    }
  ...
  async _quickFinishFeeding() {
    ...
    if (!amount) {
      wx.showToast({ title: '请先设置默认奶量', icon: 'none' })
      this.goToFeeding()
      return
    }
    try {
      ...
      wx.showToast({ title: `已保存${amount}ml`, icon: 'success' })
      await this._refreshHomeData()
    } catch (e) {
